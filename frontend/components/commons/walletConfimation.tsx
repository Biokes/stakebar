"use client"
import { ApiResponse, ConnectionOption, UserDTO, WalletConfirmationProps } from "@/lib/types";
...
export default function WalletConfirmation({ isOpen, setIsOpen }: WalletConfirmationProps) {
    ...
    const handleEmailSubmit = async () => {
        ...
        try {
            ...
            const response: ApiResponse<UserDTO> = await res.json();
            ...
            if (response.success && response.data) {
                if (response.data.isVerified) {
                    ...
                    const resend = await fetch(`${BASE_URL}/users/reverify/${email}`, {
                        method: "PATCH",
                        headers: { "Content-Type": "application/json" },
                    });
                    const reVerify: ApiResponse<string> = await resend.json();
                    if (reVerify.success) {
                        toast.success("Please check your email for verification");
                    } else {
                        toast.error(reVerify.message || "Failed to resend verification email");
                    }
                }
            }
            else {
                ...
                if (registerData.success) {
                    ...
                    toast.error(registerData.message || "Check your email for verification link");
                }
            }
        }
        catch (err) {
